...
#--------------------------------------------------------------------------------------------------------------------------------------------------------
def transient_detect(obsName, obsInd, refName, refQuery, refQueryPeriod, plot, thresh, filtSigma, LogFile, DataLoc):
...
#----------------------------------------------------------------------------------
# find_detTransient
# Functionality: Makes a region file for the given RA and DEC
# Input: RA, DEC
# Output: Regions file with the RA and DEc circled
#----------------------------------------------------------------------------------        
def Known_Transient(RA, DEC):
    from astropy.table import Table
    from astropy.io import fits
    import numpy as np
    from astropy.coordinates import SkyCoord
    ...
    import astropy.units as u
 #%%
      ## regions file for known transient
    skSrc= SkyCoord(RA*u.deg, DEC*u.deg, unit = 'deg', frame = 'fk5')
    transient_src = [];
    transient_src = (CircleSkyRegion(skSrc, radius=0.005 * u.deg))
    transient_src.visual.valid_keys[0] = 'red'   
    transient_src.write('transient_src.reg', format='ds9', overwrite = True)  
